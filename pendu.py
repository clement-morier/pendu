...
def pendu():
  ...
  window.blit(pygame.image.load("pendu1.png").convert(), (120, 0))
  pygame.display.flip()
  mots=[]
  o = 1
...
def ajout():
  window.blit(pygame.image.load("ajout.png").convert(),(0, 0))
  pygame.display.flip()
  motajouter=[]
  n=0
  c=0
  while True:
    ...
    if c != -2 and n < 25:
      ...
      for h in range(n):
        affichage(motajouter[h], h, n)
  motajout="\n"
  for lettre in motajouter:
    motajout+=lettre
  if check_string(motajout) == False :
    with open('mots.txt', 'a') as f:
      f.write(motajout)

def check_string(mot):
    with open("mots.txt") as temp_f:
